...
class LSTM_Attn(nn.Module):
    ...
    def generate_pad_mask(self, batch_size, max_len, caption_length):
        mask = torch.full((batch_size, max_len), fill_value=float('-inf'), dtype=torch.float, device='mps')
...
class CNN_Attn(nn.Module):
    ...
    def generate_pad_mask(self, batch_size, max_len, caption_length):
        ...
        for K in self.Ks:
            total_len -= (K-1)
        mask = torch.full((batch_size, total_len), fill_value=float('-inf'), dtype=torch.float, device='mps')
