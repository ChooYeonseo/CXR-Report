...
from dataprocess import NLMCXR
...
if __name__ == "__main__":
    ...
    dataset = NLMCXR('/Users/sean/Seans Mac Pro/Programming_Projects/AI/ChestXrayReportGen/dataxray/openi/', INPUT_SIZE, view_pos=['AP','PA','LATERAL'], max_views=MAX_VIEWS, sources=SOURCES, targets=TARGETS)
    ...
    checkpoint_path_from = '/Users/sean/Seans Mac Pro/Programming_Projects/AI/ChestXrayReportGen/checkpoints/{}_{}_{}.pt'.format(DATASET_NAME,MODEL_NAME,COMMENT)
    checkpoint_path_to = '/Users/sean/Seans Mac Pro/Programming_Projects/AI/ChestXrayReportGen/checkpoints/{}_{}_{}.pt'.format(DATASET_NAME,MODEL_NAME,COMMENT)
    ...
    if PHASE == 'TRAIN':
        ...
        for epoch in range(last_epoch+1, EPOCHS):
            print('Epoch:', epoch)
            train_loss = train(train_loader, model, optimizer, criterion, device='cpu', kw_src=KW_SRC, kw_tgt=KW_TGT, kw_out=KW_OUT, scaler=scaler)
            val_loss, val_outputs, val_targets = test(val_loader, model, criterion, device='cpu', kw_src=KW_SRC, kw_tgt=KW_TGT, kw_out=KW_OUT)
            test_loss, test_outputs, test_targets = test(test_loader, model, criterion, device='cpu', kw_src=KW_SRC, kw_tgt=KW_TGT, kw_out=KW_OUT)
            scheduler.step()
